...
let cita = arr[Math.floor(Math.random() * arr.length)]
  await conn.sendButton(m.chat, `Nihh ${command}nya @${m.sender.split('@')[0]}`, cita, [[`Next`, `${usedPrefix}${command}`]], m, {mentions: [m.sender], jpegThumbnail: await(await fetch(cita)).buffer()})
}
